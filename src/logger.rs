...
impl Logger {
    pub fn init_log(env: LogStage) {
        match env {
            LogStage::Testing => {
                log4rs::init_file("conf/log/log_test.toml", Default::default()).expect("init test log failure")
            }
            LogStage::Development => {
                log4rs::init_file("conf/log/log_config.toml", Default::default()).expect("init dev log failure")
            }
            LogStage::Production => {
