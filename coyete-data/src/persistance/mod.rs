...
pub fn init_pg_pool_mgr() -> PgPool {
    dotenv().ok();

    let db_url_path = env::var("DATABASE_URL").expect("DATABASE_URL must be set");
    let config = Config::default();
    let manager = ConnectionManager::<PgConnection>::new(db_url_path);
    Pool::new(config, manager).expect("Postgres pool connection error")
}
